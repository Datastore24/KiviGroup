...
- (instancetype)initWithContent: (UIView*) view andArray: (NSArray*) array
{
    ...
    if (self) {
        ...
        if (isiPhone6) {
            ...
            mainTableView.frame = CGRectMake(0, 20, self.frame.size.width, self.frame.size.height - 40);
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSDictionary * dictCell = [mainArray objectAtIndex:indexPath.row];
    [[SingleTone sharedManager] setIdentifierSubCategory:[dictCell objectForKey:@"id"]];
